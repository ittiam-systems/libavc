...
/**
**************************************************************************
* @brief argument mapping construct
**************************************************************************
*/
typedef struct
{
...
/**
**************************************************************************
* @brief list of supported arguments
**************************************************************************
*/
static const argument_t argument_mapping[] =
{
        ...
        { "--", "--disable_deblock_level", DISABLE_DEBLOCK_LEVEL,
                   "Disable deblock level - 0 : Enables deblock completely, "
                   "1: enables for I and 8th frame , 2: Enables for I only, "
                   "3 : disables completely\n" },
...
*******************************************************************************
*
* @brief Returns malloc data. Ideally should return aligned memory
*
* @param[in] alignment
*  Alignment
*
...
*******************************************************************************
*
* @brief handles unrecoverable errors. Prints error message to console and exits.
*
* @param[in] pc_err_message
*  error message
*
* @returns none
*
...
*******************************************************************************
*
* @brief Maps input string to color format
*
* @param[in] value
*  string
*
* @returns color format ID
*
* @remarks If the input string is not recognized, display a message and choose
*  YUV420P as color format
*
*******************************************************************************
*/
IV_COLOR_FORMAT_T get_chroma_fmt(CHAR *value)
{
    IV_COLOR_FORMAT_T e_chroma_format = IV_YUV_420P;
...
*******************************************************************************
*
* @brief Maps input string to a speed preset
*
* @param[in] value
*  string
*
* @returns speed preset ID
*
* @remarks If the input string is not recognized, display a message and choose
*  IVE_FASTEST as speed preset
*
*******************************************************************************
*/
IVE_SPEED_CONFIG get_speed_preset(CHAR *value)
{
    IVE_SPEED_CONFIG e_enc_speed_preset = IVE_FASTEST;
...
*******************************************************************************
*
* @brief Maps input string to a argument
*
* @param[in] value
*  string
*
* @returns argument ID
*
* @remarks If the input string is not recognized, returns INVALID
*
*******************************************************************************
*/
...
*
* @param[in] value
*  value corresponding to the argument
*
* @returns none
*
* @remarks none
...
void read_cfg_file(app_ctxt_t *ps_app_ctxt, FILE *fp_cfg)
{
    while(1)
    {
        CHAR line[STRLENGTH] = {'\0'};
        CHAR argument[STRLENGTH] = {'\0'};
        CHAR value[STRLENGTH];
        CHAR description[STRLENGTH];

        if(NULL == fgets(line, STRLENGTH, fp_cfg)) return;
...
*******************************************************************************
*
* @brief validate configured params
*
* @param[in] ps_app_ctxt
*  pointer to application context
*
...
*******************************************************************************
*
* @brief initialize default params
*
* @param[in] ps_app_ctxt
*  pointer to application context
*
...
/**
*******************************************************************************
* @brief configure input dimensions
*******************************************************************************
*/
                    ...
                    UWORD32 u4_timestamp_high)
{
    ...
    s_frame_dimensions_ip.s_ive_ip.u4_size = sizeof(ih264e_ctl_set_dimensions_ip_t);
    ...
    status = ih264e_api_function(ps_app_ctxt->ps_enc, &s_frame_dimensions_ip,
...
/**
*******************************************************************************
* @brief configure source & target framerates
*******************************************************************************
*/
                    ...
                    UWORD32 u4_timestamp_high)
{
    ...
    status = ih264e_api_function(ps_app_ctxt->ps_enc, &s_frame_rate_ip,
                                 &s_frame_rate_op);
    if(status != IV_SUCCESS)
    {
        CHAR ac_error[STRLENGTH];
                    ...
                    UWORD32 u4_timestamp_high)
{
    ...
    s_ipe_params_ip.s_ive_ip.u4_constrained_intra_pred = ps_app_ctxt->u4_constrained_intra_pred;

    s_ipe_params_op.s_ive_op.u4_size = sizeof(ih264e_ctl_set_ipe_params_op_t);

    status = ih264e_api_function(ps_app_ctxt->ps_enc, &s_ipe_params_ip,
                                 &s_ipe_params_op);
    if(status != IV_SUCCESS)
    {
        CHAR ac_error[STRLENGTH];
                  ...
                  UWORD32 u4_timestamp_high)
{
    ...
    status = ih264e_api_function(ps_app_ctxt->ps_enc, &s_bitrate_ip,
                                 &s_bitrate_op);
    if(status != IV_SUCCESS)
    {
        CHAR ac_error[STRLENGTH];
                    ...
                    IV_PICTURE_CODING_TYPE_T e_frame_type)
{
    ...
    status = ih264e_api_function(ps_app_ctxt->ps_enc, &s_frame_type_ip,
                                 &s_frame_type_op);
    if(status != IV_SUCCESS)
    {
        CHAR ac_error[STRLENGTH];
                    ...
                    UWORD32 u4_timestamp_high)
{
    ...
    s_vbv_ip.s_ive_ip.u4_vbv_buffer_delay = ps_app_ctxt->u4_vbv_buffer_delay;
...
/**
*******************************************************************************
* @brief configure profile params
*******************************************************************************
*/
...
/**
*******************************************************************************
* @brief configure deblock params
*******************************************************************************
*/
                        ...
                        UWORD32 u4_timestamp_high)
{
    ...
    s_deblock_params_ip.s_ive_ip.u4_disable_deblock_level = ps_app_ctxt->u4_disable_deblk_level;
...
#define PEAK_WINDOW_SIZE    8
    ...
    UWORD8 *pu1_buf = NULL;
    UWORD32 u4_timestamp_low = 0, u4_timestamp_high = 0;
    ...
    {
        sprintf(ac_error, "Unable to open input file for reading: %s",
    ...
    {
        sprintf(ac_error, "Unable to open output file for writing: %s",
    ...
    if(1 == ps_app_ctxt->u4_recon_enable)
    {
        ...
        {
            sprintf(ac_error, "Unable to open recon file for writing: %s",
    ...
    if(1 == ps_app_ctxt->u4_chksum_enable)
    {
        ...
        {
            sprintf(ac_error, "Unable to open checksum file for writing: %s",
     ...
     * This makes it easy to compute PSNR without adding dependency between
     * input and recon threads
     */
    ...
    if(0 != ps_app_ctxt->u4_mb_info_type)
    {
        ...
        {
            sprintf(ac_error, "Unable to open MB info file for reading: %s",
    ...
    if(ps_app_ctxt->u4_pic_info_type)
    {
        ...
        {
            sprintf(ac_error, "Unable to open Pic info file for reading: %s",
    ...
    while(1)
    {
         ...
         * And after each row we have a stride we need to calculate
        ...
        /* Read input and other associated buffers */
        while(1)
        {
            IV_STATUS_T mb_info_status = IV_SUCCESS;
            IV_STATUS_T pic_info_status = IV_SUCCESS;
        ...
        init_raw_buf_descr(ps_app_ctxt, &s_recon_buf,
                           ps_app_ctxt->as_recon_buf[0].pu1_buf,
                           ps_app_ctxt->e_recon_color_fmt);
        ps_video_encode_ip->s_recon_buf = s_recon_buf;

        if(ps_app_ctxt->u4_psnr_enable)
        {
            init_raw_buf_descr(ps_app_ctxt, &s_inp_buf,
                               ps_app_ctxt->pu1_psnr_buf,
        ...
        /* encoder api call */
        status = ih264e_api_function(ps_enc, &ih264e_video_encode_ip,
                                     &ih264e_video_encode_op);
        if(IV_SUCCESS != status)
        {
        ...
        if(ps_video_encode_op->s_out_buf.pv_buf)
        {
            for(i = 0; i < DEFAULT_MAX_OUTPUT_BUFS; i++)
            {
                if(ps_app_ctxt->as_output_buf[i].pu1_buf
                                == ps_video_encode_op->s_out_buf.pv_buf)
                {
        ...
        /* Print stats */
        {
            ...
            switch(ih264e_video_encode_op.s_ive_op.u4_encoded_frame_type)
            {
                case IV_IDR_FRAME:
                    lookup_idx = 0;
                    break;
    ...
    /* Pic count is 1 more than actual num frames encoded, because last call is
...
/**
*******************************************************************************
* @brief Application to demonstrate codec API. Shows how to use create,
...
#endif
{
    ...
    /*************************************************************************/
    {
        ...
        status = ih264e_api_function(NULL, &s_num_mem_rec_ip, &s_num_mem_rec_op);
        if(status != IV_SUCCESS)
        {
        ...
        s_app_ctxt.u4_num_mem_rec = s_num_mem_rec_op.s_ive_op.u4_num_mem_rec;
        num_mem_recs = s_num_mem_rec_op.s_ive_op.u4_num_mem_rec;
    }

    /*************************************************************************/
    /*         Allocate and Default initialize MemRecords                    */
    /*************************************************************************/
    s_app_ctxt.ps_mem_rec = malloc(num_mem_recs * sizeof(iv_mem_rec_t));
    ...
    /* Default init memory records */
    {
        iv_mem_rec_t *ps_mem_rec = s_app_ctxt.ps_mem_rec;

        for(i = 0; i < num_mem_recs; i++, ps_mem_rec++)
        {
    ...
    /*************************************************************************/
    {
        ...
        status = ih264e_api_function(NULL, &s_fill_mem_rec_ip, &s_fill_mem_rec_op);
        if(status != IV_SUCCESS)
        {
    ...
    /*************************************************************************/
    {
        ...
        {
            ps_mem_rec->pv_base = ih264a_aligned_malloc(
    ...
    /*************************************************************************/
    {
        ...
        if(s_app_ctxt.u4_recon_enable || s_app_ctxt.u4_psnr_enable
                        || s_app_ctxt.u4_chksum_enable)
        {
    ...
    /*************************************************************************/
    {
        ...
        status = ih264e_api_function(ps_enc,
                                     (void *)&s_ctl_set_getversioninfo_ip,
    ...
    /*************************************************************************/
    {
        ...
        status = ih264e_api_function(ps_enc, &s_get_buf_info_ip,
                                     &s_get_buf_info_op);
        if(status != IV_SUCCESS)
        {
    ...
    /*************************************************************************/
    {
        ...
        status = ih264e_api_function(ps_enc, &s_retrieve_mem_ip, &s_retrieve_mem_op);
        if(status != IV_SUCCESS)
        {
